...
function renderizeList() {
  ...
  while (head !== null) {
    ...
    $item.innerHTML = `
      <div class="p-2 bg-slate-200 italic">${head.prev?.element || "-"}</div>
      <div class="p-2 bg-slate-50 font-bold">${head.element}</div>
      <div class="p-2 bg-slate-200 italic">${head.next?.element || "-"}</div>
    `;
